...
pthread_cond_t condLeitura; //condicional de leitura
pthread_cond_t condEscrita; //condicional de escrita
pthread_mutex_t mutex;
int nEscrita = 0; //numero de theads executando operação de escrita
int nLeitura = 0; //número de threads executando operação de leitura
...
void* tarefa(void* arg) {
   ...
   for(long int i=id; i<QTDE_OPS; i+=nthreads) {
      op = rand() % 100;
      if(op<98) {
	 iniciaLeitura(); /* lock de LEITURA */    
         Member(i%MAX_VALUE, head_p);   /* Ignore return value */
	 terminaLeitura();     
	 read++;
      } else if(98<=op && op<99) {
	 iniciaEscrita(); /* lock de ESCRITA */    
